...
const GetCoffee = ({ coffees }) => {
  ...
  return (
    <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 mt-20 w-full max-w-4xl mx-auto'>
      {coffees.map((coffee) => (
        <div key={coffee.id} onClick={() => handleClick(coffee)} className='cursor-pointer bg-amber-600 hover:bg-amber-700 w-48 h-48 p-4 m-2 rounded-lg flex flex-col justify-center items-center transition-colors duration-300 shadow-lg'>
